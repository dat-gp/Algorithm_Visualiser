...
def disp_min(i_min):
    # Blue line displays the minimum value
...
def disp_swap(i_min, i, delay):
    ...
    # gives the distance of the swap line: x[i_min] = gives the coordinate of line i_min
    dist = int((x[i_min]+x[i])/2)
    # display swap in the middle of the line
...
def logic_loop(arr, delay):
    ...
    for i in range(len(arr)-1):
        clock.tick(FPS)
        i_min = i                       # index of smallest element in the array
